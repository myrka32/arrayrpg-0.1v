...
window.addEventListener('load', function(){
  ...
  addString('<b>Добро пожаловать на сервер!</b>');
	alt.emit('chatloaded');
});
...
document.querySelector('#adminmessage').addEventListener('submit', function(e) {
	e.preventDefault();
	var adminmessage = msgInputLine.value;
	alt.emit('adminchatmessage', adminmessage);
	saveBuffer();
	msgInputLine.value = '';
	closeChat();
});
...
if('alt' in window) {
	...
	alt.on ("addadminMessage", addadminMessage)
